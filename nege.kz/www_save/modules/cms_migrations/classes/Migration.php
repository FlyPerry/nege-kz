...
class Migration
{
	...
	public function __construct($output = FALSE, $group = 'default',$driver_class='Driver_Mysql')
	{
    ...
    public function add_foreign_key($table,$field,$name,$fk_table,$fk_field,$on_delete="cascade",$on_update="no"){
        $this->log("Adding foreign key '$name' to table '$table'...");
        $ret = $this->driver->add_foreign_key($table,$field,$name,$fk_table,$fk_field,$on_delete="cascade",$on_update="no");
        $this->log("DONE<br />");
        return $ret;
    }

    public function drop_foreign_key($table,$name){
        $this->log("Removing foreign key '$name' from table '$table'...");
        $ret = $this->driver->drop_foreign_key($table,$name);
        $this->log("DONE<br />");
        return $ret;
    }
    ...
    public function insert_data($table,$fields,$values){
        $this->driver->insert_data($table,$fields,$values);
    }
