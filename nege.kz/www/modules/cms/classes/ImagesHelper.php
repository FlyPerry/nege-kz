...
Class ImagesHelper
{
     ...
     */
    public static function resizeto($image_source,$new_size,$dimension = "width",$bg_color='#000', $transparent = false)
    {
        if(self::isgdloaded()) {
            ...
            if ($transparent){
                return self::resizeWithTransparent($image_source,$width_dest,$height_dest);
            };
     ...
     * @return resource gd resource
     */
    public static function resizeWithTransparent($image_source,$width,$height)
    {
        if(self::isgdloaded()) {
            $width_src    = self::width($image_source);
            $height_src   = self::height($image_source);
    ...
    /**
     * Build watermark image from another image
     * @access public
     * @param  mixed    $image_source  - path to image source or gd image resource
     * @param  string   $watermark_img - path to watemark image source
     * @param  int      $alpha_level   - watemark image aplha level (default = 100)
     * @return resource gd resource
     */
    public static function watermark($image_source, $watermark_img, $position = 'random', $alpha_level = 50, $ratio = 0)
    {
        if(self::isgdloaded()) {
            $watermark_img    = self::create($watermark_img);
            $height_src       = self::height($image_source);
            $image_source     = self::create($image_source);
            $cnt              = ($ratio > 0)?$ratio / 100:1;
            $watermark_img  = ImagesHelper::resizeto($watermark_img,round($height_src * $cnt),'height', '#fff', true);

            $watermark_width  = self::width($watermark_img);
            $watermark_height = self::height($watermark_img);

            $coordinates = self::calculateposition($image_source,$watermark_img,$position);
            $coordinates['dst_y'] -= $watermark_height;
            imagecopy($image_source, $watermark_img, $coordinates['dst_x'], $coordinates['dst_y'], 0, 0, $watermark_width, $watermark_height);
            return $image_source;
        }
        else {
            return false;
