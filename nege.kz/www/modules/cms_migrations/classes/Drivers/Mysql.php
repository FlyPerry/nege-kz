...
class Drivers_Mysql extends Drivers_Driver
{
    ...
    /**
     * ALTER TABLE `categories_news` ADD FOREIGN KEY (`news_id`) REFERENCES `news` (`id`) ON DELETE CASCADE ON UPDATE NO ACTION;
     * @param $table
     * @param $field
     * @param $name
     * @param $fk_table
    ...
    public function add_foreign_key($table,$field,$name,$fk_table,$fk_field,$on_delete="cascade",$on_update="no"){
        $on_delete=$this->foreign_key_action($on_delete);
        $on_update=$this->foreign_key_action($on_update);
        $sql="ALTER TABLE `$table` ADD CONSTRAINT `$name` FOREIGN KEY (`$field`) REFERENCES `$fk_table` (`$fk_field`) ON DELETE $on_delete ON UPDATE $on_update";
        return $this->run_query($sql);
    }
    ...
    public function insert_data($table,$fields,$values){
        DB::insert($table,$fields)->values($values)->execute();
    }
}